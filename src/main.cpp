...
float get_temperature(int thermistor_pin) {
    float thermistor_voltage = (5.00 / 1023.00) * analogRead(thermistor_pin);
    float resistor_voltage = 5.00 - thermistor_voltage;
    float thermistor_resistance = thermistor_voltage / (resistor_voltage / R);
...
void update_counter_array(int array_size) {
    for (int i = 0; i < array_size; i++) {
